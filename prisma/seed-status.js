...
async function seedSystemStatus() {
    ...
    const components = [
        {
            ...
            icon: 'Globe',
            order: 1,
            status: client_1.SystemStatus.OPERATIONAL,
        },
        {
            name: 'API Endpoints',
            slug: 'api',
    ...
    for (const component of allComponents) {
        ...
        for (let i = 89; i >= 0; i--) {
            ...
            const status = randomValue < uptimePercentage
                ? client_1.SystemStatus.OPERATIONAL
                : randomValue < uptimePercentage + 0.3
                    ? client_1.SystemStatus.DEGRADED
                    : client_1.SystemStatus.DOWN;
            const downtime = status === client_1.SystemStatus.DOWN ? Math.floor(Math.random() * 60) : status === client_1.SystemStatus.DEGRADED ? Math.floor(Math.random() * 10) : 0;
